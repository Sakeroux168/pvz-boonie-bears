...
extends RefCounted
## Lane/grid occupancy, protected targets and one-shot lane insurance.
## Lanes are fixed at 5 (D022); column count comes from configuration and is
## never hard-coded in core logic.
...
func _init(p_columns: int = 9) -> void:
	lanes = FIXED_LANES
...
func place_unit(cell: Vector2i, state: Dictionary) -> bool:
	if not is_empty(cell) or String(state.get("kind", "")) != "unit":
		return false
	_cells[cell] = state.duplicate(true)
	return true
...
func remove_unit(cell: Vector2i) -> Dictionary:
	...
	if String(previous.get("kind", "")) != "unit":
		return {}
	_cells.erase(cell)
	return previous
...
func damage_protected_tree_at(cell: Vector2i, amount: int) -> bool:
	...
	if String(marker.get("kind", "")) != "protected_tree":
		return false
...
func protected_target_at(cell: Vector2i) -> Dictionary:
	...
	if String(marker.get("kind", "")) != "protected_tree":
		return {}
	return _protected_targets[String(marker["target_id"])].duplicate(true)
...
func unit_positions() -> Array[Vector2i]:
	var result: Array[Vector2i] = []
	for cell in _cells:
		if String(_cells[cell].get("kind", "")) == "unit":
			result.append(cell)
	return result
